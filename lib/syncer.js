...
var _aemsyncWithSlingOption = require('aemsync--with-sling-option');
...
class Syncer {
  ...
  setUp() {
    ...
    this.pusher = new _aemsyncWithSlingOption.Pusher(this.targets, this.pushInterval, function (err, host) {
    ...
    var promise = new Promise(function (resolve, reject) {
      Syncer.queue.push(function () {
        return new Promise(function (resolve2, reject2) {
          syncer = new Syncer(targets, 3000, function onPushEndHandler(err, host) {
            if (err) {
              reject2(err);
              reject(err);
            } else {
